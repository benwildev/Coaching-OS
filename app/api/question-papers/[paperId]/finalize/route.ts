...
import { finalizeQuestionPaper } from '@/lib/services/question-paper.service';
...
export async function POST(_request: Request, { params }: { params: Promise<{ paperId: string }> }) {
  try {
    ...
    const paper = await finalizeQuestionPaper(scope, paperId);
    return NextResponse.json({ success: true, paper });
  } catch (error) {
    return apiErrorResponse(error, '/api/question-papers/[paperId]/finalize POST');
  }
}
