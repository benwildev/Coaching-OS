...
export default function GuardianChildFeesPage({ params }: { params: Promise<{ studentId: string }> }) {
  ...
  const [summary, setSummary] = useState<{ totalBilled: number; totalPaid: number; totalDue: number } | null>(null);
  ...
  return (
    <div className="max-w-[900px] mx-auto flex flex-col gap-5">
      ...
      <section className="card rounded-2xl bg-white border border-[#dce5f0] overflow-hidden">
        {invoices.length === 0 ? (
          <div className="py-12 text-center text-[13px] text-[#64748b]">{f.empty}</div>
        ) : (
      ...
      <section className="card rounded-2xl bg-white border border-[#dce5f0] overflow-hidden">
        {payments.length === 0 ? (
          <div className="py-12 text-center text-[13px] text-[#64748b]">{p.empty}</div>
        ) : (
          <ul className="divide-y divide-[#edf1f7]">
            {payments.map((pay) => (
              <li key={pay.id} className="flex items-center justify-between gap-3 p-4">
                <div className="min-w-0">
                  ...
                  <div className="text-[12px] text-[#64748b]">{formatDhakaDate(pay.paymentDate)} · {(t.paymentMethod as Record<string, string>)?.[pay.paymentMethod] || pay.paymentMethod}</div>
                </div>
